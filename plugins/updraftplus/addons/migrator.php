...
class UpdraftPlus_Addons_Migrator {
	...
	public function pre_restore_move_in($now_done, $type, $working_dir, $info, $backup_info, $restorer, $wp_filesystem_dir) {
		...
		if (is_multisite() && empty($backup['multisite']) && ( ('plugins' == $type || 'themes' == $type)  || ('uploads' == $type && !empty($this->new_blogid) && !get_site_option('ms_files_rewriting'))) ) {
		...
		if (is_array($info) && is_multisite() && isset($info['multisite']) && !$info['multisite']) {
			...
			*/
			...
			if (!empty($elements['db'])) {
				...
				$info['addui'] .= '<select style="width:100%;" id="updraft_restore_content_to_user" name="updraft_restore_content_to_user" class="updraft_select2">';
...
// 		return $info;
	...
	private function generate_new_blogid() {
		...
		if (is_integer($create)) {
			...
			echo '<strong>'.__('New site:', 'updraftplus').'</strong> '.htmlspecialchars($url).'<br>';
...
// 			$this->content = // ??
			...
			echo '<strong>'.__('Error when creating new site at your chosen address:', 'updraftplus').'</strong>';
	...
	public function updraftplus_restored_db_table($table, $import_table_prefix) {
		...
		// If we just replaced either the blogs or site table, then populate are records of what is *now* (i.e. post-restore) in them
...
class UpdraftPlus_Addons_Migrator_RemoteSend {
	...
	public function udrpc_command_send_chunk($response, $data, $name_indicator) {
		...
		if ($start > $existing_size) {
			return $this->return_rpc_message(array('response' => 'error', 'data' => "invalid_start_too_big:${start},${existing_size}"));
		}
